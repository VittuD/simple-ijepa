...
def main(cfg: TrainConfig):
    ...
    # Model & optimizer
    model, optimizer, training_ckpt_name, encoder_ckpt_name = build_model_and_optimizer(
        cfg,
        device,
        local_rank,
        rank,
    ...
    if rank == 0:
        variant_str = (
            "baseline I-JEPA" if cfg.variant == "baseline" else "Gated I-JEPA"
        )
        print(
    ...
    for epoch in range(cfg.num_epochs):
        ...
        for step, batch in enumerate(progress_bar):
            loss, stats, images = forward_batch(
                cfg, ddp_model, batch, device, cfg.fp16_precision
            )
                ...
                if step == 0 and "gate_mlp_input_example" in stats:
                    try:
                        ...
                        save_ssim_heatmap(
                            ssim_mat,
                            ssim_png_path,
                            title=f"Token SSIM (gate_mlp input @ block {cfg.gate_layer_index}, pos {cfg.gate_location})",
                        )
                    except Exception as e:
            ...
            if rank == 0:
                ...
                if cfg.variant == "baseline":
                    ...
                    desc = (
                        ...
                        f"Closed: {closed_prob_mean*100:.1f}% | "   # <-- fixed here
                        f"EMA γ: {gamma:.4f} | "
                        f"Lr: {current_lr:.6f}"
                    )
